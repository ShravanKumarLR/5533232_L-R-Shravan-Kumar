...
int flippingMatrix(int matrix_rows, int matrix_columns, int** matrix) {
    int n = matrix_rows / 2; // Because the input is 2n × 2n
    int sum = 0;

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            int v1 = matrix[i][j];
            int v2 = matrix[i][matrix_columns - 1 - j];
            int v3 = matrix[matrix_rows - 1 - i][j];
            int v4 = matrix[matrix_rows - 1 - i][matrix_columns - 1 - j];
